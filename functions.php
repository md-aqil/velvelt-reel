...
// Check if the user has purchased any product
function user_has_any_order() {
...
// Add body class based on purchase status
add_filter( 'body_class', 'add_has_purchase_class' );
function add_has_purchase_class( $classes ) {
    if (user_has_any_order()) {
        $classes[] = 'has-purchase';
    } else {
        $classes[] = 'no-purchase';
    }
    return $classes;
...
<a href="<?= esc_url($profile_url); ?>" class="profile-link">
 <img src="<?= !empty($profile_picture) 
    ...
    : esc_url(home_url('/wp-content/uploads/2025/08/20171206_01-scaled.jpg')); ?>" 
    alt="Profile" 
...
function add_talent_taxonomy_support( $args, $post_type ) {
    if ( 'talent' === $post_type ) {
        $args['taxonomies'] = array( 'talent_type' );
    }
    return $args;
}
...
function handle_talent_update() {
    ...
    wp_update_post( $talent_post_update );
    ...
    // --- Save all the same custom fields as the submission form ---
    $meta_fields = [
        '_talent_city' => 'city', '_talent_country' => 'country', '_talent_email' => 'email', '_talent_phone' => 'phone', '_talent_age_group' => 'ageGroup', '_talent_gender' => 'gender', '_talent_height' => 'height', '_talent_height_unit' => 'heightUnit', '_talent_measurements' => 'measurements', '_talent_years_active' => 'yearsActive', '_talent_affiliation' => 'affiliation', '_talent_education' => 'education', '_talent_interested_projects' => 'interestedProjects', '_talent_willing_to_travel' => 'willingToTravel', '_talent_preferred_locations' => 'preferredLocations', '_talent_brand_collabs' => 'brandCollabs', '_talent_domain' => 'domain', '_talent_role' => 'role', '_talent_instagram' => 'instagram', '_talent_linkedin' => 'linkedin', '_talent_website' => 'website',
    ];
    foreach ( $meta_fields as $meta_key => $post_key ) {
        if ( isset( $_POST[ $post_key ] ) ) {
    ...
    if ( isset( $_POST['languages'] ) && is_array( $_POST['languages'] ) ) {
        update_post_meta( $post_id, '_talent_languages', array_map( 'sanitize_text_field', $_POST['languages'] ) );
    }
    if ( isset( $_POST['availableFor'] ) && is_array( $_POST['availableFor'] ) ) {
    ...
    if ( $portfolio_files ) {
        ...
        if (!is_array($new_attachment_ids)) $new_attachment_ids = [];

        foreach ( $portfolio_files['name'] as $key => $value ) {
            if ( $portfolio_files['name'][$key] ) {
                $_FILES['single_portfolio_image'] = [ 'name' => $portfolio_files['name'][$key], 'type' => $portfolio_files['type'][$key], 'tmp_name' => $portfolio_files['tmp_name'][$key], 'error' => $portfolio_files['error'][$key], 'size' => $portfolio_files['size'][$key] ];
                $attachment_id = media_handle_upload( 'single_portfolio_image', $post_id );
                if ( ! is_wp_error( $attachment_id ) ) { $new_attachment_ids[] = $attachment_id; }
            }
        }
...
function talent_submission_success_message_shortcode() {
    ...
    ob_start();
    ?>
    <div style="text-align: center; margin: 20px 0; padding: 20px; border: 1px solid #4CAF50; background-color: #f0fff0; border-radius: 8px; color: #333;">
...
function restrict_talent_submission_access() {
    ...
    if ( is_page_template( 'template-talent-submission.php' ) ) {
        ...
        // Check if user has purchased either product variation
        // Replace 2832 and 2833 with your actual product variation IDs
        $has_basic_plan = wc_customer_bought_product( '', $user_id, 2832 );
        $has_premium_plan = wc_customer_bought_product( '', $user_id, 2833 );

        // If user hasn't purchased any plan → block access
        if ( ! $has_basic_plan && ! $has_premium_plan ) {
            wp_redirect( site_url( '/subscription-overview' ) ); // Redirect to your pricing page
            exit;
        }
    }
...
function elementor_subscription_status_shortcode() {
    ...
    // Check if user has purchased either product variation (IDs 2832 or 2833)
    $user_id = get_current_user_id();
    $has_basic_plan = wc_customer_bought_product('', $user_id, 2832);
    $has_premium_plan = wc_customer_bought_product('', $user_id, 2833);
    
    // Display subscription indicator for users who have purchased either plan
    if ($has_basic_plan || $has_premium_plan) {
        ob_start();
        ?>
    ...
    <div class="user-dashboard-wrapper">
        <div class="user-dashboard-container">
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <?php else: ?>
                <img id="profileImage" src="<?= esc_url(get_avatar_url($user_id)) ?>" class="profile-pic" alt="Profile Picture">
                <?php endif; ?>
 ...
 */
function user_has_talent_profile($user_id = null) {
