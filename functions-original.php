    ...
    foreach ($fields as $section_title => $section_fields) {
        ...
        foreach ($section_fields as $meta_key => $label) {
            ...
            if ($meta_key === '_talent_portfolio') {
                ...
                echo '</div>';
            } elseif ($meta_key === '_talent_video_links') {
                // Handle video links display
                echo '<div class="value">';
                if (!empty($value) && is_array($value)) {
                    echo '<ul style="list-style-type: disc; padding-left: 20px;">';
                    foreach ($value as $video) {
                        if (!empty($video['url'])) {
                            echo '<li><a href="' . esc_url($video['url']) . '" target="_blank">' . esc_html($video['url']) . '</a></li>';
                        }
                    }
                    echo '</ul>';
                } else {
                    echo '<em>No video links provided.</em>';
                }
                echo '</div>';
                ...
                if ($meta_key === '_talent_willing_to_travel' || $meta_key === '_talent_brand_collabs') {
                    ...
                    echo '<div class="value"><input type="checkbox" name="' . esc_attr($meta_key) . '" value="1" ' . checked($value, '1', false) . ' /> Hidden</div>';
                } else {
                ...
                if (strpos($meta_key, '_talent_hide_') === 0) {
                    // Handle privacy settings - show as unchecked checkboxes
                    echo '<div class="value"><input type="checkbox" name="' . esc_attr($meta_key) . '" value="1" /> Hidden</div>';
                } else {
...
/**
 * Restrict talent submission if profile exists
 */
function restrict_talent_submission_if_profile_exists()
{
    if (is_page('talent-submission')) {
        if (is_user_logged_in()) {
            $user_id = get_current_user_id();
            $user = get_userdata($user_id);
            $user_roles = (array) $user->roles;

            if (in_array('talent', $user_roles)) {
                $user_profile_id = get_field('profile_id', 'user_' . $user_id);
